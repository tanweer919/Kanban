...
onMounted(() => {
  ...
  if (window.location.hash.length > 1) boardStore.selectItemById(idFromUrl, selectedBoardIndex.value)
})
boardStore.$subscribe(boardStore.save)

const firstStage = computed(() => selectedStageIndex.value == 0)
const lastStage = computed(() => selectedStageIndex.value == boards.value[selectedBoardIndex.value].stages.length - 1)
const amount = computed(() => boards.value[selectedBoardIndex.value].stages?.length)
const maxAmount = 6
...
function addStage() {
  ...
  boardStore.$patch(state => state.boards[selectedBoardIndex.value].stages.push(newStage))
}

function editStageName(stageIndex: number, name: string) {
  boardStore.$patch(state => state.boards[selectedBoardIndex.value].stages[stageIndex].name = name)
}
  ...
  setTimeout(() => {
    ...
    if (itemIndex > -1) {
      boardStore.$patch(state => {
        ...
        state.selectedItem = state.boards[selectedBoardIndex.value].stages[stageIndex].items[itemIndex]
      })
    } else {
      boardStore.$patch(state => {
        const newItem = {
        ...
        state.selectedItemIndex = state.boards[selectedBoardIndex.value]
...
function moveItem(toStageIndex: number, itemId?: string) {
  if (itemId) boardStore.selectItemById(itemId, selectedBoardIndex.value)
  boardStore.$patch(state => {

    if (state.selectedItem) {
      const newItemIndex = state.boards[selectedBoardIndex.value]
        .stages[toStageIndex]
        .items
        .push(state.selectedItem) - 1

      state.boards[selectedBoardIndex.value]
...
<template>
  <div v-if="ready" class="flex h-screen flex-row gap-6 p-6 pt-12 -mt-5 antialiased">
    <stage-container
      v-for="(stage, i) in boards[selectedBoardIndex].stages"
      :key="'stage-' + stage.name + i"
      :value="stage"
      :index="i"
      @add-item="setItemSelection(i, -1)"
      @open-item="(itemIndex) => setItemSelection(i, itemIndex)"
      @move-item="moveItem"
      @save-name="(name) => editStageName(i, name)"
    />
